...
- **Cross-article auto-retrieval**: When a conversation has no references and `user.ai_cross_article_search` is true, `_do_conversation_chat` (routers/ai.py) calls `services/retrieval.py::retrieve_relevant_articles()` to keyword-retrieve top-K recent articles (title + cached feed summary, last 7 days, user-isolated via `Feed.user_id`) and writes them as `is_auto=True` `ConversationReference` rows that flow through the same multi-article prompt path. Retrieval is wrapped in try/except and degrades to no-ref chat on any failure. The retrieval signature is the stable contract for future ②③④ candidates (digest/dedup/filter); the keyword implementation may be swapped for vectors later. See [[database-guidelines]] "Scenario: Cross-Article Auto-Retrieval Service".
