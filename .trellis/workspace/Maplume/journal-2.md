...
## Session 67: AI 内容聚合:跨文章问答自动检索

**Date**: 2026-06-21
**Task**: AI 内容聚合:跨文章问答自动检索
**Branch**: `MaplumeX/salmon`

### Summary

扩展 AI 能力从单篇到跨文章:MVP 做跨文章问答的自动检索增强。brainstorm 收敛 5 问(Q1=①自动检索/Q2=关键词/Q3=7天窗/Q4=自动触发/Q5=默认开可选关),实现 services/retrieval.py + 19 单测 + 迁移 015 + User.ai_cross_article_search 开关 + ai router 接入 _do_conversation_chat + 前端 switch + i18n。trellis-check 抓出并修了 3 个问题:update_ai_config 漏写开关(阻塞验收3)、article_summaries 多 model 累积导致 outerjoin 重复行撞唯一约束(整批 auto-ref 静默丢失)、死 import。spec 沉淀 7 节 Scenario 为 ②③④ 复用记录契约。后端 60 passed、前端 build/lint 绿;alembic upgrade 未跑(环境无 PG,静态审查通过)。
